...
- **Framework**: Electron main process + renderer UI, written in TypeScript and emitted as Node16/CommonJS-compatible JavaScript for reliable Electron main/preload startup.
- **Panel UI**: `BrowserWindow` with a frameless compact HTML/CSS/TypeScript renderer. Development launches show the panel immediately so new users do not need to hunt for the tray icon.
...
| `tsconfig.json` | Strict TypeScript build configuration for Electron main/preload code. |
| `tsconfig.renderer.json` | Strict TypeScript build configuration for browser-safe renderer output. |
| `src/main/main.ts` | Electron entry point, tray icon/menu, panel lifecycle, automatic dev panel display, global shortcuts, and IPC registration. |
